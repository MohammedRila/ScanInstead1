...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/homeowner/signup", 
    [
      ...
      body('notificationPreference').optional().isIn(['email', 'phone', 'both']).withMessage('Invalid notification preference'),
      sanitizeInput,
      validateRequest
    ],
    async (req, res) => {
    try {
      const { fullName, email, password, phone, notificationPreference } = req.body;
      ...
      // Create new homeowner with authentication
      const homeowner = await storage.createHomeownerAuth({
        fullName,
        email, 
        password,
        phone,
      ...
      // Send welcome email
      try {
        await sendHomeownerWelcomeEmail(homeowner);
      } catch (emailError) {
      ...
      res.json({
        success: true,
        message: 'Account created successfully! Welcome to ScanInstead.',
        homeowner: {
          id: homeowner.id,
  ...
  app.post("/api/homeowner/login", 
    ...
    try {
      ...
      const isPasswordValid = await bcrypt.compare(password, homeowner.password);
      if (!isPasswordValid) {
        return res.status(401).json({
          success: false,
          message: 'Invalid email or password',
      ...
      console.error('Error logging in homeowner:', error);
      res.status(500).json({
        success: false,
